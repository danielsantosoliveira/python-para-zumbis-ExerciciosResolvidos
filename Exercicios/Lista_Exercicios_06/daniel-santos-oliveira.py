...
def papagaio(falando, hora):
  return falando and (hora < 7 or hora > 20) # resolução do professor
...
def apaga(s, n):
  return s[:n]+ s[n+1:] #s[:n] mostra os valores de 0 até n + s[n+1:] mostra de n+1 até o final
...
def troca(s):
  if len(s) <= 1:
    return s
  return s[-1:]+s[1:-1]+s[:1] # s[-1:] retorna a última letra + s[1:-1] as letras entre a primeira e a última + s[:1] a primeira letra
