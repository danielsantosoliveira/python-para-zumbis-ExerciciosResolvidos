...
def maior_ponta(nums):
  if nums[-1] > nums[0]:
    lista1 = [nums[-1] for x in range(len(nums))]
    return lista1
  lista0 = [nums[0] for x in range(len(nums))]
  return lista0
...
def date_fashion(eu, par):
  if (eu > 7 or par > 7) and (eu > 2 and par > 2):
    return 2
  elif eu > 2 and par > 2:
    return 1
  return 0
...
def squirrel_play(temp, is_summer):
  if (temp >= 60 and temp <= 100) and is_summer:
    return True
  elif (temp >= 60 and temp <= 90) and not is_summer:
    return True
  return False
...
def pego_correndo(speed, is_birthday):
  if (speed <= 65 and is_birthday) or (speed <= 60 and not is_birthday):
    return 0
  elif (speed <= 85 and is_birthday) or (speed <= 80 and not is_birthday):
    return 1
  return 2
...
def alarm_clock(day, vacation):
  if (day == 0 or day == 6) and vacation:
    return 'off'
  elif ((day > 0 and day < 6) and vacation) or ((day == 0 or day == 6) and not vacation):
    return '10:00'
  return '7:00'
