...
# lucky_sum(1, 13, 3) -> 1
def lucky_sum(a, b, c):
  if (a == 13 and b == 13 and c == 13) or a == 13:
    return 0
  elif b == 13:
    return a
  elif c == 13:
...
def count_code(s):
  cont = s.count('code')+s.count('cole')+s.count('core')+s.count('coze')+s.count('cope')
  return cont
...
def soma_na_lista(n, lista):
  if (n == lista[0]+lista[1] and lista[0] != lista[1]) or (n == lista[1]+lista[2] and lista[1] != lista[2]) or (n == lista[2]+lista[3] and lista[2] != lista[3]):
    return True
  return False
...
def fila_tijolos(n_peq, n_gra, meta):
  if ((n_peq * 1 + n_gra * 5) >= meta) and n_peq >= meta % 5 :
    return True
  return False
