...
def questao_A(x):
    ...
    op = x
    for t in op:
        if (t[0] == 'z' or t[0] == 'm' or t[0] == 'b') and (t[-1] != 'z' and t[-1] != 'm' and t[-1] != 'b'):
            cont += 1
    return f'Texto escolhido contém {cont} PREPOSIÇÕES.'

def questao_B(x):
    cont = 0
    op = x
    for t in op:
        if (len(t) == 7) and (t[-1] != 'z' and t[-1] != 'm' and t[-1] != 'b'):
            cont += 1
    return f'Texto escolhido contém {cont} VERBOS.'

def questao_C(x):
    cont = 0
    op = x
    for t in op:
        if (len(t) == 7) and (t[-1] != 'z' and t[-1] != 'm' and t[-1] != 'b') and (t[0] != 'z' and t[0] != 'm' and t[0] != 'b'):
            cont += 1
    return f'Texto escolhido contém {cont} VERBOS em primeira pessoa.'

def questao_D(x):
    googlon = 'zmbtshjpnwlrcxkqvdgf'
    op = x
    return sorted(x, key=lambda w: [googlon.index(i) for i in w])

def questao_E():
    return
#complexa, tentarei resolver depois.
